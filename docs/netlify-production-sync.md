...
The incident root cause is therefore confirmed as Netlify account credit exhaustion at the time of the `c40568ed...` Production event. Git integration remained connected and Preview deploys were healthy before the credit incident; the skipped Production deploy was not replayed automatically after credits were restored.
...
### 2026-09-17 Deploy Preview dependency recovery

The first head of recovery PR #42 exposed an additional Preview-only blocker before any merge was attempted:

- PR #42 head: `bb59316477495e82e2662ac3d5c3dbe39fe7f3b8`
- GitHub CI #354 reached `pnpm build` successfully
- Netlify Deploy Preview `6aabe09ae08cfe0008711d77` failed during `building site` with build script exit code 2
- Netlify plugin state remained `success`
- Netlify Preview Smoke #319 failed while waiting for the exact-head Deploy Preview; later smoke stages were skipped
- Supabase project `ivmz-home-preview` (`dzdgbkjmomvmexcbjyin`) was found in `INACTIVE` state
- Production Supabase project `ivrm-core` remained `ACTIVE_HEALTHY`
- the Preview Supabase project was restored and reached `ACTIVE_HEALTHY` before the next PR head was created

The Netlify error payload does not by itself prove the inactive Preview database was the sole build failure cause. However, GitHub's local/container build succeeded while the Netlify Preview environment failed and its configured Preview database was inactive, so the database state is a concrete Preview-environment blocker that must be removed before retrying.

The retry is generated by this real incident-record update on the same PR branch; no manual/API/MCP Netlify deploy is used. A successful exact-head Deploy Preview after the database restore is required before the PR can advance to Ready.
...
Historical exact-head Deploy Previews demonstrate that the project can build from Git events. The current recovery PR must independently regain a healthy exact-head Preview after the inactive Preview database is restored.
