  ...
  return (
    <main id="main-content" className="route-page">
      <PageHero
        ...
        description={
          <p>
            リリース・告知・イベント・publication・activityを、長文記事とは分けて追えるようにします。
          </p>
        }
      ...
      <PageSection
        ...
        description={<p>公開済みNewsだけを新しい順に表示します。</p>}
      >
        {content.state === 'error' ? (
          <EmptyState title="News is temporarily unavailable.">
            CMS queryに失敗しました。このrouteだけ安全に縮退しています。
          </EmptyState>
        ) : content.items.length === 0 ? (
          <EmptyState title="No published news yet.">
            CMSは正常です。公開されたNewsがここに並びます。
          </EmptyState>
        ) : (
            ...
            {content.items.map((item) => (
              <article className="content-row" key={item.id}>
                ...
                <div className="content-row-meta">
                  ...
                  <a href={`/news/${encodeURIComponent(item.slug)}`}>Read update →</a>
                  {item.externalUrl && (
      ...
      <PageCTA
        ...
        body="外部へ見せてよいイベント・公開・リリース予定はScheduleへ分離しています。"
        href="/schedule"
        label="Open Schedule"
      />
    </main>
